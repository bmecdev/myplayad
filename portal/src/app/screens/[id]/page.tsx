...
export default function ScreenDetailPage() {
  ...
  // Dates for assigning now
  const [assignStartDate, setAssignStartDate] = useState('');
  const [assignEndDate, setAssignEndDate] = useState('');
  
  // Dates for future scheduling
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                      ...
                      {activeGameSchedule.endDate && (
                        <p className="text-sm text-slate-400 flex items-center gap-2">
                          <Calendar className="w-4 h-4" /> 
                          Finaliza: {new Date(activeGameSchedule.endDate).toLocaleString()}
                        </p>
                      )}
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground mt-1 flex flex-col gap-1">
                        <span className="flex items-center gap-1">
                          <Calendar className="w-3 h-3" /> Inicio: {new Date(schedule.startDate).toLocaleString()}
                        </span>
                        {schedule.endDate && (
                          <span className="flex items-center gap-1 text-slate-400">
                            <Calendar className="w-3 h-3" /> Fin: {new Date(schedule.endDate).toLocaleString()}
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <div className="glass rounded-2xl w-full max-w-md p-6 shadow-2xl border border-white/10">
            ...
            <form onSubmit={handleAssignGame} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-gray-400">Inicio (opcional)</label>
                  <input 
                    type="datetime-local" 
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/50"
                    value={assignStartDate}
                    onChange={e => setAssignStartDate(e.target.value)}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-gray-400">Fin (opcional)</label>
                  <input 
                    type="datetime-local" 
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/50"
                    value={assignEndDate}
